...
module FindAI
  ...
  class BaseClient
    ...
    # @private
    #
    # @return [FindAI::PooledNetRequester]
    ...
    #
    #   @option req [FindAI::Converter, Class, nil] :model
    #
    ...
    private def send_request(request, redirect_count:, retry_count:, send_retry_header:)
      ...
      in (400..) if retry_count >= max_retries || (response && !self.class.should_retry?(
        ...
        decoded = FindAI::Util.decode_content(response, stream: stream, suppress_error: true)
    ...
    private def parse_response(req, headers:, stream:)
      decoded = FindAI::Util.decode_content(headers, stream: stream)
      unwrapped = FindAI::Util.dig(decoded, req[:unwrap])

      case [req[:page], req.fetch(:model, FindAI::Unknown)]
      in [Class => page, _]
        page.new(client: self, req: req, headers: headers, unwrapped: unwrapped)
      in [nil, Class | FindAI::Converter => model]
        FindAI::Converter.coerce(model, unwrapped)
      in [nil, nil]
        unwrapped
      end
    end
