...
module FindAI
  ...
  class BaseClient
    ...
    # @param opts [Hash{Symbol => Object}, FindAI::RequestOptions]
    #
    # @raise [ArgumentError]
    def validate_request(req, opts)
      ...
      case opts
      in FindAI::RequestOptions | Hash
        valid_keys = FindAI::RequestOptions.options
    ...
    def resolve_uri_elements(req)
      from_args =
        if (url = req[:url])
          FindAI::Util.parse_uri(url)
        else
          path = FindAI::Util.normalize_path("/#{@base_path}/#{req.fetch(:path)}")
          req.slice(:scheme, :host, :port, :query).merge(path: path)
        end

      query = FindAI::Util.deep_merge(
        from_args[:query] || {},
        req[:extra_query] || {},
        concat: true
      )
    ...
    def prep_request(options)
      ...
      if [:get, :head, :options].include?(method)
        ...
        body = FindAI::Util.deep_merge(body, extra_body)
      end
    ...
    def retry_delay(response, retry_count:)
      ...
      span =
        if retry_header
          FindAI::Util.suppress(ArgumentError) do
    ...
    #
    # @raise [FindAI::HTTP::Error]
    # @return [Hash{Symbol => Object}]
    def follow_redirect(request, status:, location_header:)
      uri = FindAI::Util.unparse_uri(request)
      location =
        FindAI::Util.suppress(ArgumentError) do
      ...
      # from undici
      if FindAI::Util.uri_origin(uri) != FindAI::Util.uri_origin(location)
    ...
    def request(req, opts)
      ...
      options = FindAI::Util.deep_merge(req, opts)
      ...
      if page
        ...
        FindAI::Converter.convert(model, raw_data)
      else
        raw_data
      end
    end
    ...
    # @return [String]
    def inspect
      base_url = FindAI::Util.unparse_uri(scheme: @scheme, host: @host, port: @port, path: @base_path)
